<div class="row invoice-info">
    ...
    <div class="col-sm-6 invoice-col mb-3">
        <form action="{{ route('update.invoice') }}" method="POST" id="form_update">
            ...
            <input type="hidden" name="cs" value="{{ $sale_erp->pic->name }}">
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <x-select2 name="pic" id="pic" data-placeholder="Pilih PIC">
                        ...
                        @continue
                        @endif
                        @if (empty($tl->fullname))
                        @continue
                        @endif
                        <option value="{{ $tl->id }}" {{ $sale_erp->pic->id == $tl->id ? 'selected' : '' }}>{{ $tl->fullname }}</option>
                        @endforeach
                        @foreach ($cs_erp as $cs)
                        <option value="{{ $cs->id }}" {{ $sale_erp->pic->id == $cs->id ? 'selected' : '' }}>{{ $cs->fullname }}</option>
                        @endforeach
                    </x-select2>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#pic').change(function() {
            $('[name="cs"]').val(this.options[this.selectedIndex].innerHTML);
        })
