<div class="modal fade" id="updateAdmin{{ $loop->index }}" tabindex="-1" role="dialog" aria-labelledby="outletTitle" aria-hidden="true">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.update', $admin->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-7">
                            <x-input type="text" name="name" value="{{ old('name') ?? $admin->name }}" />
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="username" class="col-sm-5 col-form-label">Username</label>
                        <div class="col-sm-7">
                            <x-input type="text" name="username" value="{{ old('username') ?? $admin->username }}" />
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <x-select name="role">
                                @foreach ($roles as $role)
                                    <option value="{{ $role->id }}" @if ($role->id == $admin->role) selected @endif>{{ $role->user_role }}</option>
                                @endforeach
                            </x-select>
                        </div>
                    </div>
